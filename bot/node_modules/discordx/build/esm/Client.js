            ...
            this.applicationCommands.map((DCommand, index) => {
                ...
                const printOptions = (options, depth) => {
                    ...
                    options.forEach((option, optionIndex) => {
                        this.logger.log(`${(option.type === ApplicationCommandOptionType.Subcommand ||
                            option.type ===
                            ...
                            ? option.name
                            : option.name}: ${ApplicationCommandOptionType[option.type]?.toLowerCase()} (${option.classRef.name}.${option.key})`);
            ...
            this.simpleCommands.map((cmd) => {
                ...
                const printOptions = (options, depth) => {
                    ...
                    options.forEach((option) => {
                        this.logger.log(`${tab}${option.name}: ${SimpleCommandOptionType[option.type] ?? "unknown"} (${option.classRef.name}.${option.key})`);
                    });
                };
