    ...
    sendUsageSyntax() {
        ...
        embed.addFields({
            ...
            value: `\`\`\`${this.prefix.toString()}${this.name} ${this.info.options
                .map((op) => `{${op.name}: ${index_js_1.SimpleCommandOptionType[op.type] ?? "unknown"}}`)
                .join(" ")}\`\`\``,
        });
