...
class Client extends discord_js_1.Client {
    ...
    async initGuildApplicationCommands(guildId, DCommands, options) {
        ...
        await Promise.all(DCommands.map(async (DCommand) => {
            ...
            const isEqual = lodash_1.default.isEqual(lodash_1.default.omit(commandJson, "id", "applicationId", "guild", "guildId", "version"), rawData);
            if (!isEqual) {
                commandToUpdate.push(new index_js_1.ApplicationCommandMixin(findCommand, DCommand));
            }
        }));
        ...
        const addOperation = options?.disable?.add
            ? []
            : added.map(async (DCommand) => guild.commands.create(await DCommand.toJSON(new index_js_1.ApplicationGuildMixin(guild, DCommand))));
        const updateOperation = options?.disable?.update
            ? []
            : commandToUpdate.map(async (cmd) => cmd.command.edit(await cmd.instance.toJSON(new index_js_1.ApplicationGuildMixin(guild, cmd.instance))));
    ...
    async initGlobalApplicationCommands(options) {
        ...
        if (AllCommands) {
            ...
            await Promise.all(DCommands.map(async (DCommand) => {
                ...
                const isEqual = lodash_1.default.isEqual(lodash_1.default.omit(findCommand.toJSON(), "id", "applicationId", "guild", "guildId", "version"), rawData);
                if (!isEqual) {
                    commandToUpdate.push(new index_js_1.ApplicationCommandMixin(findCommand, DCommand));
                }
            }));
    ...
    async initGuildApplicationPermissions(guildId, DCommands, log) {
        ...
        ApplicationCommands.forEach((cmd) => {
            ...
            if (findCommand) {
                commandToUpdate.push(new index_js_1.ApplicationCommandMixin(cmd, findCommand));
            }
        });
        await Promise.all(commandToUpdate.map((cmd) => {
            return guild.commands.permissions
                ...
                if (!lodash_1.default.isEqual(permissions, commandPermissions)) {
                    if (log ?? !this.silent) {
    ...
    async executeComponent(components, interaction, log) {
        ...
        const guilds = [];
        if (component) {
            guilds.push(...(await (0, index_js_1.resolveIGuilds)(this, component, [
                ...botResolvedGuilds,
                ...component.guilds,
            ])));
        }
    ...
    async executeContextMenu(interaction, log) {
        ...
        const guilds = [];
        if (applicationCommand) {
            guilds.push(...(await (0, index_js_1.resolveIGuilds)(this, applicationCommand, [
                ...botResolvedGuilds,
    ...
    async executeCommand(message, options) {
        ...
        // validate guild id
        const commandGuilds = await (0, index_js_1.resolveIGuilds)(this, command, [
            ...botResolvedGuilds,
            ...command.info.guilds,
        ]);
        if (message.guild?.id &&
